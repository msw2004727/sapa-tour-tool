...
5. **畫面上不能出現「領隊」或「導遊」**：這團是自由行。管理功能叫「管理模式／管理專區」，廣播叫「即時廣播」，說明文字要找人時寫「主辦人」。`audit/wording.js` 會擋。
...
| `audit/hltest.js` | 卡片高亮提醒的狀態機與版面；v3.21 起實際點畫面：每張卡收起不亮、重新展開繼續亮，航班卡回程階段重新亮 |
...
### 9b. 時間有兩個時區，第 1 天搭機前是台灣時間

所有「現在幾點」都用越南時間（`tzParts(VN)`），但**出發日搭機前人還在台灣**：05:30 機場集合是台灣時間。v3.18 以前把它當越南時間算，出發日清晨的集合倒數**多了 1 小時**（05:20 顯示「還有 1 小時 10 分」，其實只剩 10 分）。

現在的規則（`isTWSlot` / `twItem`）：出發日當天、時間不晚於最晚一班去程起飛（`settings.flights.*.out`，目前 09:00）→ 台灣時間，其餘越南時間。倒數與下一站一律換成絕對時間（`slotMs`）再比，不要再用「時:分」字串直接比大小。

「現在」一律用 `nowMs()` 取得，不要直接寫 `new Date()`——時間模擬靠它。資料的 `_ts` 仍用真實的 `Date.now()`。

### 9c. 時間模擬與「手動指定第幾天」的差別

| | 時間模擬（v3.19） | 團務設定「手動指定第幾天」 |
|---|---|---|
| 影響範圍 | **只有這支手機**（`sessionStorage['sapa-sim']` 或網址 `?sim=2026-09-25T09:00`，台灣時間） | **全團每一支手機**（存在雲端 `settings.dayOverride`） |
| 可以看出發前／回國後 | 可以 | 不行（只能指定旅途中第幾天） |
| 存檔 | 模擬期間 `Store.enqueue` 與 `Store.cache` 直接略過，不寫雲端也不寫手機 | 正常存檔 |

要測試就用時間模擬；手動指定只在行程整天延後這種真實狀況才用。
...
| `homeKey` + `tick` | 日期或下一站變了才重畫首頁；表單開著或正在輸入時不動；換天時行程頁自動回到今天（v3.19） |
| `bcActive`（v3.19） | 廣播的集合時間過了 4 小時就視為過期，首頁大字卡改回「今天下一站」；資料本身不刪 |
| `bcDate`（v3.19） | 沒有日期的舊廣播，用 `_ts`（存檔時間）推回是哪一天，不會每天復活 |
| `flightPhase`（v3.20） | 航班卡分段：出發前與第 1 天 `out`（只顯示去程）、第 2～(天數−2) 天 `none`（收起，釘住也收）、倒數第 2 天起與回國後 `back`（只顯示回程）。首頁「報到航廈」只在 `out` 顯示。**小麥指定不要同時列出去回程** |
