/* 用詞與住宿卡 LINE 按鈕
   這團是「團體自由行」，沒有領隊。任何畫面、按鈕文字、aria-label 都不能出現「領隊」。
...
(async()=>{
  ...
  console.log('[1] 所有畫面都不出現「領隊」（團員與管理模式都查）');
  const r=await p.evaluate(()=>{
    ...
    const check=(label)=>{ screens++;
      const t=document.body.innerText; const i=t.indexOf('領隊');
      ...
      document.querySelectorAll('[aria-label],[title],[placeholder]').forEach(el=>{
        ...
        if(a.indexOf('領隊')>=0) found.push(label+'（屬性）：'+a); }); };
  ...
  ck('沒有任何畫面出現「領隊」',r.found.length===0,r.found.slice(0,5));
